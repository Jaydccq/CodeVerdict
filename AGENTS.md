...
**CodeVerdict** - this checkout is currently being operated as a private LeetCode-style practice platform. NestJS 11 backend + Vue 3 frontend, served as a single container. Judge0-backed execution remains available, but the default local Docker path is the lightweight practice app plus PostgreSQL, with bundled Judge0 opt-in only.
...
```bash
npm run docker:up           # Default local practice stack
npm run docker:up:judge0    # Opt-in bundled Judge0 profile
npm run docker:down
...
Multi-stage Dockerfile: client → server → `node:22-alpine`. Serves Vue as static files via `express-static-gzip` (Brotli). Runs as non-root user. Health check on `/api/health`.
...
- Bundled Judge0 is not the default local path anymore. Use `npm run docker:up` for normal practice development and `npm run docker:up:judge0` only when you explicitly want the local Judge0 profile.
